...
function loadFavApi(i){
    ...
    .then((data) => {
        ...
        <div class="topRatedSection"><div class="hiddenJsonForDetails">${JSON.stringify(data)}</div><div class="sliderPageForTopRated"><img src="${data.image}"><div class="sliderPageInvisibleLayer"><h3 class="sliderPageTitle">${data.name}</h3></div></div><div class="topRatedFooter"><div class="topRatedPrice">${data.salePrice}$</div><div class="sliderDetailsBtn">Details</div><div class="sliderDetailsBtn removeFav" data-id="${arr[i]}">Remove</div></div></div>
        `;
        count++;
        if(count<arr.length){
        loadFavApi(count);
